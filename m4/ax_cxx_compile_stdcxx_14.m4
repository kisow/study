...
#   -std=c++1y).  If neither is specified, you get whatever works, with
...
#   Copyright (c) 2014 Gyusik Choe <gyusik.choe@navercorp.com>
#
...
m4_define([_AX_CXX_COMPILE_STDCXX_14_testbody], [[
  template <typename T>
    ...
    decltype (auto) e = static_cast<int&&>(a);
]])
...
AC_DEFUN([AX_CXX_COMPILE_STDCXX_14], [dnl
  ...
  m4_if([$2], [], [dot_cxx_compile_cxx14_required=true],
        [$2], [mandatory], [dot_cxx_compile_cxx14_required=true],
        [$2], [optional], [dot_cxx_compile_cxx14_required=false],
  ...
  dot_cv_cxx_compile_cxx14,
  [AC_COMPILE_IFELSE([AC_LANG_SOURCE([_AX_CXX_COMPILE_STDCXX_14_testbody])],
    [dot_cv_cxx_compile_cxx14=yes],
    [dot_cv_cxx_compile_cxx14=no])])
  if test x$dot_cv_cxx_compile_cxx14 = xyes; then
    ac_success=yes
  fi
  ...
  if test x$ac_success = xno; then
    for switch in -std=gnu++14 -std=gnu++1y; do
      cachevar=AS_TR_SH([dot_cv_cxx_compile_cxx14_$switch])
  ...
  AC_LANG_POP([C++])
  if test x$dot_cxx_compile_cxx14_required = xtrue; then
    if test x$ac_success = xno; then
