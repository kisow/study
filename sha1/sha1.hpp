...
struct Sha1 : public std::array<uint32_t, 5>
{
	Sha1(const Sha1& src)
	{
	...
	static void hash(const uint8_t* msg, uint32_t length, uint32_t* h) /*{{{*/
	{
		h[0] = 0x67452301;
		h[1] = 0xEFCDAB89;
		h[2] = 0x98BADCFE;
		...
		uint64_t longLen = ((uint64_t)length) << 3;
		for(i = 0; i < 8; i++) {
